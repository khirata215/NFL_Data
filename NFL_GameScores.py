...
def NFL_GameScores(Year = this_yr, Team = 'ALL'):
    ...
    if Team == 'ALL':
        ...
        for team in team_abbr:
            ...
            # Assigning the score to the away team
            curr_team['AwayTeamScore'] = np.select(
                [curr_team.location == 'Away'], 
                [curr_team.points_scored], 
                default = curr_team.points_allowed
                )

            # Assigning the score to the home team
            curr_team['HomeTeamScore'] = np.select(
                [curr_team.location == 'Home'], 
                [curr_team.points_scored], 
                default = curr_team.points_allowed
                )
        ...
        # Use SportsReference's Schedule function to pull the current team's games
        all_games = Schedule(abbreviation = Team, year = Year).dataframe[
                [
                'datetime',
                'week',
                'location',
        ...
        # Figuring out how much the away team scored
        all_games['AwayTeamScore'] = np.select(
            [all_games.location == 'Away'], 
            [all_games.points_scored], 
            default = all_games.points_allowed
            )

        # Figuring out how much the home team scored
        all_games['HomeTeamScore'] = np.select(
            [all_games.location == 'Home'], 
            [all_games.points_scored], 
            default = all_games.points_allowed
            )
